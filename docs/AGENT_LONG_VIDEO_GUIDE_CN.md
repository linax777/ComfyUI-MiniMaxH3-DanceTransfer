# MiniMax H3 Dance 时间线导演台：长视频生成 Agent 操作规范
...
本规范解决的是 Agent 如何使用 MiniMax H3 Dance 时间线导演台编排长视频。插件负责组织当前一次生成的参考素材、固定 Guide、音频和标签；Agent 仍然负责全片规划、逐段提示词、质量筛选、循环生成以及最终去重叠合并。
